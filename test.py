...
def drag(rowcoltab1, rowcoltab2):
    ...
    m.move(x0 + x1, y0 + cfg.YTOPCANVAS + y1)
    m.press(x0 + x1, y0 + cfg.YTOPCANVAS + y1)
    cfg.canvas.after(100, cfg.win.update())
    m.move(x0 + x2, y0 + cfg.YTOPCANVAS + y2)
    cfg.canvas.after(100, cfg.win.update())
    m.release(x0 + x2, y0 + cfg.YTOPCANVAS + y2)
    cfg.canvas.after(100, cfg.win.update())

def click(rowcoltab):
    print("Test clicks {} ".format(rowcoltab))
    x, y = cfg.board.off_to_pixel(rowcoltab)
    m.move(x0 + x, y0 + cfg.YTOPCANVAS + y)
    m.press(x0 + x, y0 + cfg.YTOPCANVAS + y, 1)
    #cfg.canvas.after(100, cfg.win.update())
    m.release(x0 + x, y0 + cfg.YTOPCANVAS + y, 1)
    #m.click(x, y, 1)
    return x, y